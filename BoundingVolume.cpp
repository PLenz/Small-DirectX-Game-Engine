...
BoundingVolume::BoundingVolume(std::vector<Vertex>& vertices, std::vector<DWORD>& indicies)
{
	...
	if (m_vertices.size() > 0) {
		...
		for (int i = 1; i < m_vertices.size(); ++i) {
			if (m_vertices[i].x < m_bounding_min.x) m_bounding_min.x = m_vertices[i].x;
			if (m_vertices[i].y < m_bounding_min.y) m_bounding_min.y = m_vertices[i].y;
			if (m_vertices[i].z < m_bounding_min.z) m_bounding_min.z = m_vertices[i].z;

			if (m_vertices[i].x > m_bounding_max.x) m_bounding_max.x = m_vertices[i].x;
			if (m_vertices[i].y > m_bounding_max.y) m_bounding_max.y = m_vertices[i].y;
			if (m_vertices[i].z > m_bounding_max.z) m_bounding_max.z = m_vertices[i].z;
		}
	}
...
bool BoundingVolume::Update(XMFLOAT3* position, XMFLOAT4* rotation)
{
	if ((position->x != m_position.x || position->y != m_position.y || position->z != m_position.z) ||
		(rotation->x != m_rotation.x || rotation->y != m_rotation.y || rotation->z != m_rotation.z || rotation->w != m_rotation.w))
	{
		...
		for (int i = 0; i < 8; ++i) {
			XMStoreFloat3(&m_rotatedVertices[i], XMVector3Rotate(XMLoadFloat3(&m_rotatedVertices[i]), XMLoadFloat4(rotation)));
		...
		for (int i = 1; i < 8; ++i) {
			if (m_rotatedVertices[i].x < m_min.x) m_min.x = m_rotatedVertices[i].x;
			if (m_rotatedVertices[i].y < m_min.y) m_min.y = m_rotatedVertices[i].y;
			if (m_rotatedVertices[i].z < m_min.z) m_min.z = m_rotatedVertices[i].z;

			if (m_rotatedVertices[i].x > m_max.x) m_max.x = m_rotatedVertices[i].x;
			if (m_rotatedVertices[i].y > m_max.y) m_max.y = m_rotatedVertices[i].y;
			if (m_rotatedVertices[i].z > m_max.z) m_max.z = m_rotatedVertices[i].z;
		}
...
bool BoundingVolume::IntersectsExactly(BoundingVolume* other, XMFLOAT3 & resolution)
{
	...
	for (int normalIndex = 0; normalIndex < m_normals.size(); normalIndex++)
	{
		...
		for (int vectorIndex = 0; vectorIndex < otherVertices.size(); vectorIndex++)
		{
			...
			float projection = XMVectorGetX(XMVector3Dot(XMLoadFloat3(&currentOtherVector), XMLoadFloat3(&currentNormal))) - currentOffset;
...
{	
	for (XMFLOAT3 elem : m_rotatedVertices) vertices.push_back(XMFLOAT3(elem));
}
