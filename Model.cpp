...
bool Model::PopulateCommandList(ComPtr<ID3D12GraphicsCommandList>& commandList, UINT8* constantBufferGPUAddress,
...
bool Model::Intersects(BoundingVolume& cameraBounds, XMFLOAT3& resolution) {
  if (m_solid) {
    ...
    if (cameraBounds.IntersectsAlongAxes(meshBoundingVolume, resolution)) {
      if (cameraBounds.IntersectsExactly(meshBoundingVolume, resolution) && meshBoundingVolume->IntersectsExactly(
        &cameraBounds, resolution)) {
        return true;
      }
    }
  }
...
void Model::SetCull(bool cullModel, ComPtr<ID3D12GraphicsCommandList>& commandList,
                    ComPtr<ID3D12CommandAllocator>& commandAllocator) {
